...
SAMPLE_DIR="$ROOT_DIR/app/light_bulb_local"
BUILD_DIR="${BUILD_DIR:-$ROOT_DIR/build/light_bulb}"
BOARD="${BOARD:-nice_nano_v2/nrf52840/uf2}"
OVERLAY_FILE="$ROOT_DIR/overlays/light_bulb_nice_nano.overlay"
PYTHON_BIN="${PYTHON_BIN:-/usr/bin/python3}"
CACHE_DIR="${CACHE_DIR:-$ROOT_DIR/.cache}"

if [[ ! -d "$SAMPLE_DIR" ]]; then
  echo "No encuentro el sample Zigbee en: $SAMPLE_DIR" >&2
  exit 1
fi
